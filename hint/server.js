...
app.get('/ejs',(req,res) => {
        ...
        MongoClient.connect(url, function(err, db) {
                if (err) throw err;
                var dbo = db.db("dev");
                dbo.collection("users").find({},{created_at:1,text:1,'user.name':1}).toArray(function(e                                                                                                           rr, result) {
                        if (err) throw err;
                        twit = result
                        //console.log(twit)
                        res.render('test01',{name:'hint',data:twit})
                        db.close()
                })
        })
        //console.log(twit)
...
app.get('/', function (req, res) {
    res.send('<h1>Login Form</h1> <br> <form method="post" action="/showuser"> username: <input type="t                                                                                                           ext" name="username"> <br> password: <input type="text" name="password"> <input type="submit"> </form>'                                                                                                           );
});
